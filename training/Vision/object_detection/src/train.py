...
"""
Addding flags for class labels, Batch_size, custom image dimensions (width & height) 
"""
tf.app.flags.DEFINE_spaceseplist('classes', None,
                            """List of class names for this object detection""",
                                 comma_compat=True)
tf.app.flags.mark_flag_as_required('classes')

tf.app.flags.DEFINE_float('learning_rate', 0.0001,
                            """Initial learning rate for the training rate.""")
tf.app.flags.DEFINE_integer('batch_size', 20,
                            """Batch size of images that will be used during training.""")
tf.app.flags.DEFINE_integer('image_width', 64,
                            """Desired width of the image for training"""
                            """(default 64)""")
tf.app.flags.DEFINE_integer('image_height', 64,
                            """Desired height of the image for training"""
                            """(default 64)""")

FLAGS.classes = [clsname.replace("_"," ") for clsname in FLAGS.classes]
...
def train():
  ...
  with tf.Graph().as_default():
    ...
    if FLAGS.net == 'vgg16':
      ...
      mc.IS_TRAINING = True
      mc.CLASS_NAMES = FLAGS.classes
      mc.CLASSES = len(mc.CLASS_NAMES)
      mc.BATCH_SIZE = FLAGS.batch_size
      mc.IMAGE_WIDTH = FLAGS.image_width
      mc.IMAGE_HEIGHT = FLAGS.image_height
